/*
 * stand-alone macro to draw photon trigger turn on curves.
 */
...
const float cutPhoEta  = 1.44;
const bool  doHoverE = false;

void drawTurnOn(const TString inputFile, const TString outputFile = "drawTurnOn.root", const TString outputFigureName = "");
double calcTLegendHeight(int nEntries);
double calcTLegendWidth(std::string label);

void drawTurnOn(const TString inputFile, const TString outputFile, const TString outputFigureName)
{
    std::cout<<"running drawTurnOn()"<<std::endl;
    std::cout<<"inputFile   = "<< inputFile.Data() <<std::endl;
    std::cout<<"outputFile  = "<< outputFile.Data() <<std::endl;

    // triggers that go into numerator
    std::vector<std::string> triggerBranchesNum {"HLT_HISinglePhoton10_Eta1p5_v1",
    ...
    std::vector<std::string> triggerBranchesDenom {"HLT_HISinglePhoton10_Eta1p5_v1"
    };
    int nTriggersDenom = triggerBranchesDenom.size();

    std::vector<std::string> inputFiles {inputFile.Data()};
    ...
    treeggHiNtuplizer->SetBranchAddress("nPho",&nPho);
    treeggHiNtuplizer->SetBranchAddress("phoE",&phoE);
    treeggHiNtuplizer->SetBranchAddress("phoEt",&phoEt);
    treeggHiNtuplizer->SetBranchAddress("phoEta",&phoEta);
    treeggHiNtuplizer->SetBranchAddress("phoPhi",&phoPhi);
    treeggHiNtuplizer->SetBranchAddress("pho_ecalClusterIsoR4",&pho_ecalClusterIsoR4);
    treeggHiNtuplizer->SetBranchAddress("pho_hcalRechitIsoR4",&pho_hcalRechitIsoR4);
    treeggHiNtuplizer->SetBranchAddress("pho_trackIsoR4PtCut20",&pho_trackIsoR4PtCut20);
    treeggHiNtuplizer->SetBranchAddress("phoR9",&phoR9);
    treeggHiNtuplizer->SetBranchAddress("phoHoverE",&phoHoverE);
    treeggHiNtuplizer->SetBranchAddress("phoSigmaIEtaIEta",&phoSigmaIEtaIEta);
    treeggHiNtuplizer->SetBranchAddress("phoSigmaIEtaIEta_2012",&phoSigmaIEtaIEta_2012);
    treeggHiNtuplizer->SetBranchAddress("phoSCRawE",&phoSCRawE);
    treeggHiNtuplizer->SetBranchAddress("phoE5x5",&phoE5x5);
    treeggHiNtuplizer->SetBranchAddress("phoSCEtaWidth",&phoSCEtaWidth);
    treeggHiNtuplizer->SetBranchAddress("phoSCPhiWidth",&phoSCPhiWidth);
    treeggHiNtuplizer->SetBranchAddress("pho_swissCrx",&pho_swissCrx);
    treeggHiNtuplizer->SetBranchAddress("pho_seedTime",&pho_seedTime);
    treeggHiNtuplizer->SetBranchAddress("pfcIso4",&pfcIso4);
    treeggHiNtuplizer->SetBranchAddress("pfnIso4",&pfnIso4);
    ...
    for (Long64_t j_entry = 0; j_entry < entries; ++j_entry)
    {
        ...
        for (int i=0; i<nPho; ++i) {
            ...
            if (!(TMath::Abs(phoEta->at(i)) < cutPhoEta)) continue;
            if (!(phoSigmaIEtaIEta->at(i) > 0.002 && pho_swissCrx->at(i) < 0.9 && TMath::Abs(pho_seedTime->at(i)) < 3)) continue;
            if (doHoverE) {
                if (!(phoHoverE->at(i) < 0.1)) continue;
            }
    ...
    TLegend* leg = new TLegend(0.1, 0.9-height, 0.1 + width, 0.9, "Triggers");
...
{
    if      (argc == 4)    drawTurnOn(argv[1], argv[2], argv[3]);
    else if (argc == 3)    drawTurnOn(argv[1], argv[2]);
    else if (argc == 2)    drawTurnOn(argv[1]);
    else {
        std::cout << "Usage : \n" <<
                "./drawTurnOn.exe <inputFile> <outputFile> (<outputFigureName>)"
                << std::endl;
        return 1;
    }
    return 0;
